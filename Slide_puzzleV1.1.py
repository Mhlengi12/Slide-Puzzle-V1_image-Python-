...
class SlidePuzzle:
    ...
    def draw(self,screen):
        ...
        #Create buttons
        cfont = pygame.font.Font('freesansbold.ttf',25)
        TextX = 535 
        TextY = 530
        btext = cfont.render('Breadth First Solve' ,True, (0,0,0) ,(255,255,255))   
        ctext = cfont.render('Best First Solve' ,True, (0,0,0) ,(255,255,255))
        screen.blit(btext,(TextX,TextY))
        screen.blit(ctext,(TextX,TextY+30))
...
#Prints cords of tiles(current & defualt)        
    ...
    def setTile(self):
        with open('Tile_position.csv','r') as file:
            
            file_reader = csv.DictReader(file) ####!!!!!!!!!!!!!!!!!
            #load csv values into array
            for line in file_reader:
                if int(line['Tile'])-1 == 8:
                    self.opentile = (int(line['Xindex']),int(line['Yindex']))
                else:
                    self.tiles[int(line['Tile'])-1] = (int(line['Xindex']),int(line['Yindex']))
...
while running:
    ...
    #Sets tiles to sepcific positions
    if first_time:
        program.setTile()
        first_time = False
